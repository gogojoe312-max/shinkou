...
function productionTasksSheet(s,group='all'){
 ...
 const h=groups.map(g=>'<section class="production-task-section"><h3>'+esc(g.label)+'</h3>'+r.nodes.filter(n=>n.group===g.id).map(n=>productionTaskRow(s,n)).join('')+'</section>').join('')+(group==='all'&&r.extra.length?'<section class="production-task-section"><h3>その他の登録済み作業</h3>'+r.extra.map(x=>'<button class="production-legacy-row" data-production-legacy="'+esc(x.k)+'"><span>'+esc(plainStage(x.n))+'</span><small>'+(s.stages?.[x.k]?.done?'完了':s.stages?.[x.k]?.st?'対応中':'記録を確認')+' ›</small></button>').join('')+'</section>':'')+(group==='instrument'?'<label class="quick-field">この曲の楽器録音<select class="inp" id="productionInstruments" aria-label="楽器録音の必要性">'+[['unknown','必要か未確認'],['required','必要'],['none','不要']].map(([v,l])=>'<option value="'+v+'" '+((s.production?.instruments||'unknown')===v?'selected':'')+'>'+l+'</option>').join('')+'</select></label>':'')+(group==='chorus'?'<label class="quick-field">この曲のコーラス<select class="inp" id="productionChorus" aria-label="コーラスの必要性"><option value="required">あり（通常）</option><option value="none" '+(s.production?.chorus==='none'?'selected':'')+'>なし</option></select></label>':'');
...
function productionTaskEditor(s,id,back='all'){
 ...
 let h='<label class="quick-field">状態<select class="inp" id="productionState" aria-label="状態">'+Object.entries(ShinkouProduction.STATES).filter(([k])=>k!=='na'||!n.keys.length).map(([k,v])=>'<option value="'+k+'" '+(n.state===k?'selected':'')+'>'+v+'</option>').join('')+'</select></label>';
 ...
 h+='<label class="quick-field">連絡手段<select class="inp" id="productionChannel" aria-label="連絡手段"><option value="">未設定</option><option value="email" '+(n.channel==='email'?'selected':'')+'>メール</option><option value="line" '+(n.channel==='line'?'selected':'')+'>LINE</option></select></label>';
 h+=field('締切・予定日','productionDue',n.due.kind==='target'?'':n.due.value,'date')+'<label class="quick-field">日程の状態<select class="inp" id="productionDueKind" aria-label="日程の状態">'+[['registered','登録日（確定状況未確認）'],['tentative','仮'],['confirmed','確定']].map(([v,l])=>'<option value="'+v+'" '+(n.due.kind===v?'selected':'')+'>'+l+'</option>').join('')+'</select></label>';
